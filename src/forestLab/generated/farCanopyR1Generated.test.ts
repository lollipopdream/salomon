...
const PNG_URL = new URL('./far-canopy-r1-2048.png', import.meta.url);
const META_URL = new URL('./far-canopy-r1-meta.json', import.meta.url);
...
describe('generated FAR canopy R1 appearance texture (I-11)', () => {
  ...
  it('is tagged as the R1 appearance and keeps the locked world alignment', () => {
    expect(meta.appearance).toBe('R1');
  ...
  it('records the family-driven appearance parameters used for cell/color selection', () => {
    expect(meta.appearanceParameters.familyThresholds.DARK_CONIFER_THRESHOLD).toBe(0.40);
    expect(meta.appearanceParameters.familyThresholds.BRIGHT_BROADLEAF_THRESHOLD).toBe(0.60);
  ...
  it('matches all three recorded input digests to the source files (identical inputs to BASELINE)', () => {
